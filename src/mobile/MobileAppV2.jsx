...
export default function MobileAppV2({ onDesktopVersion }) {
  ...
  async function removeItem(item){
    if(!window.confirm(`確認已驗收「${titleOf(item)}」？將從 AI Work 清單移除，成果與學習紀錄會保留。`)) return
    setBusy(item.id)
    ...
    <main className="mobile-content">
      ...
      {tab==='work' && <>
        ...
        {(waitingPlans.length>0||needsInput.length>0)&&<section><div className="mobile-section-title"><h2>🔴 等你處理</h2><span>{approvals}</span></div><div className="mobile-list">
          {waitingPlans.map(p=><article className="mobile-approval" key={p.id}><span><ShieldCheck size={16}/>{ownerLabel(p.owner)} 準備開始</span><strong>{titleOf(p)}</strong>{p.whyNow&&<small>{p.whyNow}</small>}<div className="mobile-proposal-evidence"><small><b>觀察：</b>{p.observation||p.context||'待補充'}</small><small><b>建議：</b>{p.suggestedAction||p.proposedSteps?.[0]||'先完成必要查證'}</small><small><b>成果：</b>{p.expectedOutcome||'產出可驗證成果'}</small>{(p.evidenceRefs||[]).filter(ref=>String(ref).startsWith('http')).map(ref=><a key={ref} href={ref} target="_blank" rel="noreferrer">查看研究來源</a>)}</div><div><button disabled={busy===p.id} onClick={()=>decide(p,'approve')}>同意</button><button disabled={busy===p.id} onClick={()=>decide(p,'approve_with_judgment')}>同意＋自行判斷</button><button disabled={busy===p.id} onClick={()=>decide(p,'reject')}>取消</button></div></article>)}
          {needsInput.map(w=><article className="mobile-approval" key={w.id}><span><Clock3 size={16}/>{ownerLabel(w.owner)} 需要補充</span><strong>{titleOf(w)}</strong><small>{w.clarificationQuestion||w.error||'AI 需要你的方向才能繼續。'}</small><textarea value={drafts[w.id]||''} onChange={e=>setDraft(w.id,e.target.value)} placeholder="直接回答，送出後 AI 自動續跑" className="mobile-work-textarea"/><div><button disabled={busy===w.id||!(drafts[w.id]||'').trim()} onClick={()=>answer(w)}>回覆並繼續</button><button disabled={busy===w.id} onClick={()=>removeItem(w)}><Trash2 size={14}/>刪除</button></div></article>)}
        </div></section>}
