...
import { ArrowLeft, RefreshCw, Plus, ExternalLink, Folder, Trash2 } from 'lucide-react'
...
export default function AIWorkPilot() {
  ...
  return <div className="pilot-page text-slate-800">
    ...
    {!selected ? <>
      ...
      <div className="pilot-tools"><div className="pilot-filters my-7 flex gap-2 border-b">{[['all','全部'],...Object.entries(STATUS).map(([key,value])=>[key,value[0]])].map(([key,label])=><button key={key} onClick={()=>setFilter(key)} className={`px-4 py-3 text-sm ${filter===key?'border-b-2 border-blue-600 text-blue-700':''}`}>{label}</button>)}</div><button disabled={busy || data.migrationRequired} className="pilot-primary" onClick={()=>setShowCreate(true)}><Plus className="inline mr-2" size={18}/>新增 AI Work</button></div>
      <div className="pilot-list overflow-x-auto rounded-2xl border bg-white"><div className="min-w-[980px]"><div className="pilot-columns grid grid-cols-[2fr_110px_120px_1.4fr_1.4fr_100px] gap-4 border-b px-6 py-4 text-sm"><span>專案名稱 / 目標</span><span>負責 Bot</span><span>狀態</span><span>目前狀況</span><span>下一步</span><span>狀態變更</span></div>{visible.map(p=>{const presentation = getProjectPresentation(p); return <button key={p.id} onClick={()=>open(p.id)} className="pilot-row grid w-full grid-cols-[2fr_110px_120px_1.4fr_1.4fr_100px] items-center gap-4 border-b px-6 py-5 text-left"><span className="pilot-project"><span className="pilot-thumb bg-blue-50">{presentation.imageUrl ? <img src={presentation.imageUrl} alt={presentation.imageAlt} loading="lazy" width="83" height="83"/> : <Folder size={28} aria-hidden="true"/>}</span><span><b>{p.title}</b><small className="pilot-goal-summary mt-1 block text-slate-500">{presentation.goalSummary}</small><small className="text-blue-600">{p.phase}{p.currentMilestoneId ? ` · ${p.currentMilestoneId} / ${p.projectPlan.milestones.length}` : ''}</small></span></span><span>{BOTS[p.responsibleBot] || BOTS[p.owner]}</span><span>{badge(p)}</span><span className="text-sm">{Array.from(p.currentState || '').length > 50 ? `${Array.from(p.currentState).slice(0, 49).join('')}…` : p.currentState}</span><span className="text-sm">{p.nextAction}{p.humanAction && <strong className="mt-2 block text-blue-700 underline">前往確認 →</strong>}</span><span className="text-xs text-slate-500">{date(p.statusChangedAt)}</span></button>})}{!visible.length && <p className="p-6 text-slate-500">{data.loaded?'目前沒有符合條件的專案。':'讀取中…'}</p>}</div></div>
    </> : <>
      ...
      {review && <section aria-label="需要你處理" className="my-5 rounded-2xl border-2 border-amber-300 bg-amber-50 p-6">
        <h3 className="text-lg font-semibold">需要你處理：{review.title}</h3>
        <p className="mt-2 text-sm text-slate-600">規劃版本 {review.planVersion} · {review.effect}</p>
        {review.document && <details open className="mt-4"><summary className="cursor-pointer font-medium">查看待確認規劃全文</summary><pre className="pilot-document mt-3">{review.document}</pre></details>}
        {review.summary && <p className="my-4 whitespace-pre-wrap">{review.summary}</p>}
        {(review.deliverables || []).map((a,i)=><a key={i} className="pilot-open my-3 block" href={a.url || a.uri || a.commit} target="_blank" rel="noopener noreferrer">{a.title || a.name || '開啟成果'} <ExternalLink size={14}/></a>)}
        {review.kind==='clarification' ? <>
          <ul className="my-4 list-disc space-y-2 pl-5">{review.questions.map((q,i)=><li key={i}>{q}</li>)}</ul>
          <textarea aria-label="補充資料" className="pilot-field" value={answer} onChange={e=>setAnswer(e.target.value)} placeholder="直接回答上面的問題"/>
          <button disabled={busy || !answer.trim()} className="pilot-primary mt-3" onClick={async()=>{if(await decide('answer',answer))setAnswer('')}}>{review.confirmLabel}</button>
        </> : <div className="mt-4 space-y-3">
          <button disabled={busy} className="pilot-primary" onClick={()=>decide(review.kind==='plan'?'approve_plan':review.kind==='delivery'?'acknowledge_result':'approve_result')}>{review.confirmLabel}</button>
          <textarea aria-label="修改意見" className="pilot-field" placeholder="需要修改的內容" value={feedback} onChange={e=>setFeedback(e.target.value)}/>
