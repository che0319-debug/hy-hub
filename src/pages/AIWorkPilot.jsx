...
export default function AIWorkPilot() {
  ...
  return <div className="pilot-page text-slate-800">
    ...
    {!selected ? <>
      ...
      {tab === 'Google Drive' && <div className={`${panel} pilot-card`}><h3 className="font-semibold">Google Drive</h3><p className="pilot-drive-note">此 TEST 案的獨立資料夾與成果檔案</p><a className="pilot-folder" target="_blank" rel="noopener noreferrer" href={folderUrl(selected)}><Folder size={20}/>開啟 {selected.testCode} 資料夾 <ExternalLink size={16}/></a>{(!selected.driveFolderId || selected.artifacts.some(a => !a.driveFileId)) && <button className="pilot-sync" disabled={busy} onClick={() => run(() => sendAIWorkPilotEvent(selected.id, 'drive_sync', {}))}>同步 Drive 索引</button>}{artifacts(selected)}</div>}
