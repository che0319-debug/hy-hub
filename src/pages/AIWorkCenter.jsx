...
function projectRow(project) {
  ...
  const result = hasResult ? `${project.latestResultMilestoneId ? `${project.latestResultMilestoneId} · ` : ''}${resultSummary || deliverable.name || '開啟成果'}` : '尚無成果'
...
export default function AIWorkCenter() {
  ...
  const latestResultIsCurrent = selected.currentMilestoneId && selected.latestResultMilestoneId === selected.currentMilestoneId
  ...
  return <div className="mx-auto max-w-5xl">
    ...
    {tab === 'overview' ? <div className="mt-6 space-y-5">
      ...
      <section className="rounded-2xl border bg-white p-6"><h2 className="font-semibold text-slate-900">最新成果{selected.latestResultMilestoneId ? ` · ${selected.latestResultMilestoneId}` : ''}</h2>{deliverable ? <><button type="button" onClick={() => setReportOpen(value => !value)} className="mt-4 flex items-center gap-2 text-left text-blue-700 underline"><FileText size={16}/>{deliverable.title || deliverable.name || '檢視成果'}（{reportOpen ? '收合' : '展開'}）</button>{reportOpen && <div className="mt-5 border-t pt-5"><p className="text-sm text-slate-600">{latestResultSummary || '成果內容載入中，請按「同步」更新。'}</p></div>}</> : latestResultSummary ? <p className="mt-3 whitespace-pre-wrap text-sm text-slate-600">{latestResultSummary}</p> : <p className="mt-3 text-sm text-slate-400">尚未產生可開啟 Deliverable。</p>}</section>
