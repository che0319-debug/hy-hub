...
import { approveWorkspaceMilestone, approveWorkspacePlan, archiveWorkspaceProject, createWorkspacePlanRevision, createWorkspaceProject, retryWorkspaceDriveFolder, fetchWorkspace, requestWorkspacePlanAnalysis, saveWorkspacePlan, submitWorkFeedback, updateWorkspaceResponsibleBot } from '../lifeOSApi'
...
export default function AIWorkCenter() {
  ...
  return <div className="mx-auto max-w-5xl">
    ...
    {tab === 'overview' ? <div className="mt-6 space-y-5">
      {selected.driveProvisioningStatus && <section className="rounded-2xl border bg-white p-6"><h2 className="font-semibold text-slate-900">Google Drive 與參考資料</h2>{selected.driveProvisioningStatus === 'ready' ? <div className="mt-3 flex flex-wrap gap-5 text-sm"><a href={selected.driveFolderUrl} target="_blank" rel="noopener noreferrer" className="text-blue-700 underline">開啟專案資料夾</a><a href={selected.referenceFolderUrl} target="_blank" rel="noopener noreferrer" className="text-blue-700 underline">開啟 01_參考資料</a></div> : <div className="mt-3 text-sm text-amber-800"><p>專案已建立；Drive 資料夾尚未連線。正式成果暫不可標示為已上傳。</p><button disabled={busy} className="mt-3 rounded-lg border border-amber-300 px-3 py-2" onClick={async () => { setBusy(true); setError(''); try { const result = await retryWorkspaceDriveFolder(selected.id); await load(); setNotice(result.project?.driveProvisioningStatus === 'ready' ? 'Drive 資料夾已建立。' : '後端 Drive 權限不足；專案保持待連線。') } catch (e) { setError(e.message || '重試失敗') } finally { setBusy(false) } }}>重試連結 Drive</button></div>}</section>}
      ...
      {plan.approvalStatus === 'approved' && <div className="border-t bg-blue-50 p-6 text-sm text-blue-800">規劃書已核准。請到「專案總覽」查看目前里程碑進度。</div>}
