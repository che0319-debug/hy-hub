...
export default function AIWorkCenter() {
  ...
  return <div className="mx-auto max-w-5xl">
    ...
    {tab === 'overview' ? <div className="mt-6 space-y-5">
      <section className="rounded-2xl border bg-white p-6"><h2 className="font-semibold text-slate-900">目前進度</h2><div className="mt-4 space-y-2">{milestones.map(item => <div key={item.id} className={`rounded-xl border p-4 ${item.id === selected.currentMilestoneId ? 'border-blue-400 bg-blue-50/40' : 'bg-white'}`}><div className="flex justify-between gap-3"><b>{item.id}　{item.title}</b><span className="text-xs text-slate-500">{item.status === 'completed' ? '✓ 完成' : item.id === selected.currentMilestoneId ? '目前' : '待執行'}</span></div></div>)}</div><p className="mt-5 text-sm text-slate-600"><b>目前正在做：</b>{['queued','claimed','running','in_progress'].includes(selected.planAnalysis?.status) ? '等待 AI 解析規劃書' : current ? `${current.id} ${current.title}` : plan.approvalStatus === 'approved' ? '等待下一階段' : '等待規劃書核准'}</p><p className="mt-2 text-sm text-slate-600"><b>下一步：</b>{selected.workspaceStatus === 'review' ? '檢視成果，選擇同意驗收或送出修改意見' : ['queued','claimed','running','in_progress'].includes(selected.planAnalysis?.status) ? '由 AI 接手處理，目前不需要你的操作' : plan.approvalStatus === 'approved' ? '依規劃書產出可開啟成果' : '檢視並核准規劃書'}</p></section>
      <section className="rounded-2xl border bg-white p-6"><h2 className="font-semibold text-slate-900">最新成果</h2>{deliverable ? <a href={deliverable.url || deliverable.uri} target="_blank" rel="noreferrer" className="mt-4 flex items-center gap-2 text-blue-700"><ExternalLink size={16}/>{deliverable.name || '開啟成果'}</a> : latestResultSummary ? <p className="mt-3 whitespace-pre-wrap text-sm text-slate-600">{latestResultSummary}</p> : <p className="mt-3 text-sm text-slate-400">尚未產生可開啟 Deliverable。</p>}</section>
      <section className="rounded-2xl border bg-white p-6"><h2 className="font-semibold text-slate-900">驗收／修改意見</h2><textarea value={feedback} onChange={e => setFeedback(e.target.value)} placeholder="若成果需要修改，請輸入對本 Milestone 的要求" className="mt-4 min-h-28 w-full rounded-xl border p-3 text-sm"/><div className="mt-3 flex flex-wrap gap-2"><button onClick={submitFeedback} disabled={busy || !feedback.trim() || !selected.latestWork?.id} className="flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2 text-sm text-white disabled:opacity-40"><Send size={15}/>送出修改意見</button>{selected.workspaceStatus === 'review' && selected.latestWork?.status === 'succeeded' && <button onClick={acceptMilestone} disabled={busy} className="flex items-center gap-2 rounded-lg bg-emerald-600 px-4 py-2 text-sm text-white disabled:opacity-40"><CheckCircle2 size={15}/>同意驗收</button>}</div></section>
