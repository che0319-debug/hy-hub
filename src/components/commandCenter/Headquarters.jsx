...
export default function Headquarters({offices,work,groups,connection,now,detailed,selection,onOffice,onAgent,travelRequest,onTravelState,roaming}){
  ...
  function tick(time){
   ...
   if(!document.hidden){
    for(const [id,journey] of Object.entries(journeys.current)){
     ...
     if(journey.distance>=journey.total){locations.current[id]=journey.destination;delete journeys.current[id];el?.classList.remove('is-walking','travel-paused');setTravel(t=>{const n={...t};delete n[id];return n});onTravelState(`${offices.find(o=>o.id===id)?.name} 已抵達 ${offices.find(o=>o.id===journey.destination)?.name} Office · 空間移動`)}
    }
